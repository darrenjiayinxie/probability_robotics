...
def get_heuristic(cell, goal):
    ...
    h2 = [1, 2, 5, 10]
    heuristic = h2[3] * np.linalg.norm(cell - goal)
...
def plot_expanded(expandeds, start, goal):
    ...
    plt.plot(expandeds[:, 0], expandeds[:, 1], 'yo', markersize = 5)
...
def run_path_planning(occ_map, start, goal):
    ...
    animation.save('dijkstra.gif', writer='imagemagick')
...
def main():
    ...
    occ_map = np.loadtxt('map2.txt')

    # start and goal position [x, y]
    start = np.array([80, 54])
    goal = np.array([100, 80])
